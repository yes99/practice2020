        ...
        while cap.isOpened():  # 캡쳐 객체 초기화 확인
            ...
            if key == 32 or cnt == 1:
                ...
                while back < hold:
                    ...
                    rewindframenum = "./origin/origin%d.png" %back
                    rewindframe = cv2.imread(rewindframenum)
                    


                    if  back == hold - 10:
                        cv2.namedWindow('image')
                        cv2.imshow('image', rewindframe)
                        x_down1, y_down1, x_up1, y_up = -1, -1, -1, -1
                        cv2.setMouseCallback('image', onMouse, param=rewindframe)

                        while True:
                            rk = cv2.waitKey(delay) & 0xFF
                            # 비트연산자 & 로 둘다 1인것만 1 운영체제가 64비트라 이런 과정을 해줘야된대
                            if rk == 27:
                                break
                            # 다시 영역지정
...
#                                frame = img_draw.copy()
                    ...
                    newframe =Optical_flow(rewindframe)
                    cv2.imshow('result', newframe)

                    cv2.imshow('result', rewindframe)
                    cv2.waitKey(1)
                    convertframe = "convert%d.png" % back
                    cv2.imwrite('./convert/'+convertframe, rewindframe)
                    back = back +1
                    print("되감기중 되감기중되감기중 되감기중되감기중 되감기중되감기중 되감기중되감기중 되감기중되감기중 되감기중되감기중 되감기중")
