...
def Optical_flow(frame,cnt):
    global xi, yi, x_copy, y_copy, numOfDot1, stopbegin , stopbegincnt, cornerflag
...
def Optical_flow2(frame,cnt):
    global xi, yi, x_copy, y_copy, x2, y2, x_copy2, y_copy2, numOfDot1, numOfDot2, stopbegin , stopbegincnt, cornerflag
    width = frame.shape[1]
    height = frame.shape[0]
...
class MyWindow(QDialog, form_class):
    ...
    def editStart(self):
        ...
        while cap.isOpened():  # 캡쳐 객체 초기화 확인
            ...
            if cnt == 1 or key ==32 or cornerstop ==1:
                cornerstop = 0
                countM= 0
                squares = 0
                cv2.namedWindow('image')
            ...
            if  squares == 0:
                ...
                print("박스 : 1")
                cornerstop, newframe = Optical_flow(frame, cnt)
                print("zero stop check",cornerstop)
                ...
                print("박스 : 2")
                cornerstop, newframe = Optical_flow2(frame, cnt)
                print("zero stop check",cornerstop)
                out.write(newframe)
                cv2.imwrite('frame.png', frame)
                self.qPixmapFileVar = QPixmap()
