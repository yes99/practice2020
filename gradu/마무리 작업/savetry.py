...
while cap.isOpened():  # 캡쳐 객체 초기화 확인
    ...
    #print(originframe)
    #print('./fortestorigin/'+originframe)
    cv2.imwrite('./fortestorigin/'+originframe, frame)
    origincnt +=1
    ...
    cvtframe = "convertframe%d.png" %cnt
    cv2.imshow('result', Optical_flow(frame))
    cv2.imwrite('./fortestconvert/'+cvtframe, Optical_flow(frame))
