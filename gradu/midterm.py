...
def SURF(frame):
    ...
    img2 = cv2.polylines(img2, [np.int32(new_pts)], True, 255, 1, cv2.LINE_AA)
    ...
    try:
        ...
        pass
    cutimg = img2[int(y_min):int(y_max), int(x_min):int(x_max)].copy()
    cv2.imwrite('query.jpg', cutimg)

    painting = img2[int(y_min) - 5][int(x_min) - 5]
