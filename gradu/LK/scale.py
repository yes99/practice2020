...
def Optical_flow(frame):
    ...
    if cnt > 2:
        ...
        if ratio < 0.4:
            cnt0 = "1 current%d past %d" %(numOfCurrent,numOfDot)
            print(cnt0)
            cv2.putText(img2, cnt0, (50, 400),  cv2.FONT_HERSHEY_DUPLEX, 1, (0, 200, 0), 2)
            xi = xi + 3
            yi = yi + 3
        ...
        #현재특징점이 더 많을때->로고가 커짐
            ...
            cnt0 = "2 current%d past %d" %(numOfCurrent,numOfDot)
            print(cnt0)
            cv2.putText(img2, cnt0, (50, 400),  cv2.FONT_HERSHEY_DUPLEX, 1, (0, 200, 0), 2)
            xi = xi - 2
            yi = yi - 2
...
while cap.isOpened():  # 캡쳐 객체 초기화 확인
    ...
    #print("result img")
    fcnt = fcnt +1
    putimg = Optical_flow(frame)
    cv2.imshow('result', putimg)
    cv2.imwrite("./im/frame%d.png" %fcnt, putimg)
