...
def Optical_flow(frame):
    ...
    for i,(p,n) in enumerate(zip(preMv,nextMv)):
        ...
        if px > xi and px < x_copy and py > yi and py < y_copy:
            ...
            if dx !=0:
                dxy[abs(int(dy/dx))] +=1 
                print("기울기", abs(int(dy/dx)))
            if dxy[abs(int(dy/dx))] == 4:
                print("크기 변환 감지")
                keepdx = dx
                keepdy = dy
                scaleflag = 1
    ...
    print("크기 변환 감지", scaleflag)

    print("sumX: %f counX %f sumY %f countY %f" %(sumX , countX, sumY, countY))
    ...
    if scaleflag ==0:
        ...
        xi = xi -abs(keepdx)
        x_copy = x_copy + abs(keepdx)
        yi = yi + abs(keepdy)
        y_copy = y_copy - abs(keepdy)
        print("moveX", abs(keepdx))
        print("moveY", abs(keepdy))
