...
void CreateJuliaSet();
__global__ void JuliaKernel(unsigned char* pImage, float cx, float cy, float R, int MaxIter);
...
// 전역변수
#define TILE_WIDTH 32				// 블록의 크기
const int Dim = 1024;				// 이미지 크기
unsigned char Image[Dim * Dim * 3];	// CPU 이미지 배열
unsigned char* DevImage;			// GPU 이미지 배열
float theta = 0.0;					// JuliaSet의 상수 생성을 위한 파라미터
...
void CreateJuliaSet()
{
	...
	JuliaKernel << <gridDim, blockDim >> > (DevImage, cx, cy, R, MaxIter);
...
__global__ void JuliaKernel(unsigned char* pImage, float cx, float cy, float R, int MaxIter)
{
	...
	if (x < Dim && y < Dim)
	{
		...
		int Iter = Julia(a, b, cx, cy, R, MaxIter);
