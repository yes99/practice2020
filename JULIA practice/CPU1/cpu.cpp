...
const int Dim = 1024;
unsigned char Image[Dim * Dim * 3]; // 1024 by 1024 RBG 이미지 배열
float Theta = 0.0;
int MaxIter = 256;
...
void Timer(int id)
{
	...
	Theta += 0.01;
	glutPostRedisplay(); // Render() 함수 호출...
	glutTimerFunc(30, Timer, 0);
}

void CreateJuliaSet()
{
	// 임의의 상수 복소수 정의 C = cx + cy i.
