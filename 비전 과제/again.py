...
img = cv2.imread( 'meantest1.png', cv2.COLOR_BGR2GRAY)
img2 = cv2.imread('meantest2.png', cv2.COLOR_BGR2GRAY)
draw = cv2.imread('meantest1.png', cv2.COLOR_BGR2GRAY)
...
#img2 = cv2.resize(img2, (w//10 ,h//10)) 
#draw = cv2.resize(draw, (w//10 ,h//10))
...
Zdraw= np.pad(draw, pad_width=1, mode='constant', constant_values=0)
...
#그래디언트를 위한 마스크
arrt = np.ones((3,3), np.float32)
arrx = np.ones((3,3), np.float32)
arry = np.ones((3,3), np.float32)

#기본과 그래디언트 차이 델타값구하기
dx = np.ones((3,3), np.float32)
dy = np.ones((3,3), np.float32)
dt = np.ones((3,3), np.float32)


#후계산 처리 과정
A = np.ones((9,2), np.float32)

#A의 트랜스 t
At= np.ones((2,9), np.float32)
AtA = np.ones((2,2), np.float32)
#역함수
R = np.ones((2,2), np.float32)
#A들 전체 계산 (AtA)rAt
As = np.ones((2,9), np.float32)
#B값
B = np.ones((9,1), np.float32)
#도출된 vu
...
pwidth = 3
move = 2
for y in range(1 ,h-(move-1)):     # 1~h까지 3*3이니까
    for x in range(1, w -(move-1)): # 1~w까지 3*3이니까
        arr = masking(Z, pwidth, y,x)
        arrt  = masking(Z2, pwidth, y,x)
        arrx  = masking(Z, pwidth, y,x+move)
        arry  = masking(Z, pwidth, y+move,x)
        dx = arrx - arr
        dy = arry - arr
        dt = arrt -arr
        '''
        ...
        if (AtA[0][0] * AtA[1][1]) - (AtA[1][0] * AtA[0][1]) != 0 :    #역함수는 만들수 있을까??
           ...
           # print("R\n", R)
            ...
            if( round(vu[0,0]) !=0 or  round(vu[1,0]) != 0 ):
                if(round(vu[0,0]) <3 and round(vu[1,0]) <3 ):
                    print("그리기 시작")
                    #draw = cv2.line(draw, (x,y) , (x+round(vu[0,0]), y+round(vu[1,0])), black, 1)
                    #draw = cv2.circle(draw, (x,y), 1, black  )
                    draw = cv2.circle(draw , (x+round(vu[0,0]), y+round(vu[1,0])), 1, black  )
                    #draw = cv2.arrowedLine(draw, (x,y) , (x+round(vu[0,0]), y+round(vu[1,0])), black, 2)
