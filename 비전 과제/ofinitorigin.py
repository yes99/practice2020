...
for y in range(0 ,h-1):
    for x in range(0, w-1):
        ...
        for i in range (0, 3):
            for j in range(0, 3):        
                arr[i][j] = Z[y+i][x+j]
                arr2x[i][j] = Z2[y+i][x+j+1]
                arr2y[i][j] = Z2[y+i+1][x+j]
                arr2[i][j] = Z2[y+i][x+j]
        #print(arr)
        #print(arr2x)
        #print(arr2y)      
        ...
        if (AtA[0][0] * AtA[1][1]) - (AtA[1][0] * AtA[0][1]) != 0 :      
            ...
            print(uv)
            if( int(uv[1,0]) !=0 or  int(uv[1,0]) != 0 ):
                img = cv2.line(img, (y,x) , (y+int(uv[0,0]), x+int(uv[1,0])), black, 2) 
            '''
            u = int(uv[0][0])
            v = int(uv[1][0])
            print(uv[0][0], uv[1][0])
