...
print(dB)
print("==============")
np.savetxt("save.txt", dR, fmt='%d', delimiter=',')
np.savetxt("save1.txt", dG, fmt='%d', delimiter=',')
np.savetxt("save2.txt", dB, fmt='%d', delimiter=',')
beepsound()
...
for i in range(h+1, height-h-1):
    for j in range(h+1, width-h-1):
        ...
        while(True):
            ...
            #euclidean_dist0(pointA, pointB)
            if( abs(euclidean_dist0( [i,j],[y0i,y0j]))< 3 or repeatcnt > 10):
