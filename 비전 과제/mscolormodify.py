...
#print(img[10][10])
#################################################
f1 = open("spacelog.txt", 'w')
f2 = open("colorlog.txt", 'w')
        ...
        while(True):
            ...
            while(True):
                ...
                for l in range (-10, 11):
                    for z in range (-10, 11):
                        ###  RGB
                        rlist.append(R[i-l][j-z])
                        upsumr = upsumr + R[i-l][j-z]* gaussian_k(R[i-l][j-z],R[i,j],3) #x, y, h
                        downsumr = downsumr + gaussian_k(R[i-l][j-z],R[i,j],3)           
                ytr = upsumr / downsumr
                if abs(R[rformerr1][rformerr2] - ytr)<5:
                    R[i][j] = round(ytr)
                    break
                else:
                    rlsit - 
            ...
            #print(i , j)
            log1 = "i %d, j %d,  ytr %d, ytg %d, ytb%d\n" %( i, j ,round(ytg) , round(ytr) , round(ytb) )
            f.write(log1)
